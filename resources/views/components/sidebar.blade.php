...
@php
    ...
    if ($activeModule === 'Pengguna' && !isset($allSidebarConfig[$progKey])) {
        $programSidebar = [
            'Pengguna & Hak Akses' => [
                ...
                'items' => [['label' => 'Daftar Pengguna', 'url' => '/users', 'active' => ['users*']]],
            ],
        ];
    } else {
    ...
    $defaultActiveAccordion =
        collect($programSidebar)
            ->keys()
            ->first(function ($k) use ($programSidebar, $activeModule) {
                $d = $programSidebar[$k];
                $active = $k === $activeModule;
                if (isset($d['items'])) {
                    foreach ($d['items'] as $ci) {
                        $p = strtok($ci['url'], '?');
                        if (request()->is(ltrim($p, '/')) || ($ci['url'] !== '#' && request()->url() === url($p))) {
                            $active = true;
                            break;
                        }
                        if (isset($ci['active']) && is_array($ci['active'])) {
                            foreach ($ci['active'] as $pat) {
                                if (request()->is($pat)) {
                                    $active = true;
                                    break 2;
                                }
                ...
                return $active;
            }) ?? collect($programSidebar)->keys()->first();
@endphp

<aside
    :class="sidebarOpen ? 'translate-x-0 w-[var(--sidebar-width)]' :
        '-translate-x-full lg:translate-x-0 w-[var(--sidebar-width)] lg:w-[var(--sidebar-width)]'"
    style="background-color: #E8ECF1;" :style="darkMode ? 'background-color: #0A0E17;' : 'background-color: #E8ECF1;'"
    class="fixed lg:relative z-40 h-full font-sans font-medium transition-all duration-300 shrink-0 overflow-hidden flex flex-col select-none border-0">
    ...
    <div class="p-5 shrink-0 flex flex-col gap-3 border-0">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                <div
                    class="w-9 h-9 rounded-xl bg-gradient-to-br from-teal-light to-teal-700 text-white flex items-center justify-center shadow-md shadow-teal-light/20 shrink-0 border border-teal-500/30">
                    <img src="{{ asset('assets/images/logo-kkp.png') }}" alt="Logo KKP"
                        class="w-6 h-6 object-contain bg-white/95 rounded-md p-0.5 transition-all">
                </div>
                <div class="min-w-0">
                    <h1
                        class="font-sans font-medium text-sm text-textMain-light dark:text-textMain-dark leading-tight truncate">
                        Program Prioritas
                    </h1>
                    <p
                        class="font-sans font-medium text-[11px] text-teal-light dark:text-teal-400 uppercase tracking-wider mt-0.5 truncate">
                        {{ $activeProgram }}
                    </p>
                </div>
            </div>
            <button @click="sidebarOpen = false"
                class="lg:hidden text-textMuted-light hover:text-textMain-light dark:hover:text-textMain-dark p-1">
        ...
        @if ($activeModule !== 'Pengguna')
            <div class="pt-1">
                @if (\Illuminate\Support\Facades\Auth::user()->isUserDaerah())
                    <a href="{{ url('greetings') }}"
                        class="w-full flex items-center justify-center gap-2 py-2 px-3 rounded-xl text-xs font-sans font-medium text-textMuted-light dark:text-textMuted-dark hover:text-textMain-light dark:hover:text-textMain-dark transition-all">
                        <i class="fa-solid fa-arrow-left text-xs text-teal-light"></i> Kembali ke Portal
                    </a>
                @else
                    <a href="{{ url('greetings') }}"
                        class="w-full flex items-center justify-between py-2 px-3 rounded-xl text-xs font-sans font-medium text-textMuted-light dark:text-textMuted-dark hover:text-textMain-light dark:hover:text-textMain-dark transition-all group">
                        <span class="flex items-center gap-2.5 font-sans font-medium">
                            <i
                                class="fa-regular fa-star text-amber-500 text-xs group-hover:scale-110 transition-transform"></i>
                            Ganti Program
                        </span>
                        <i
                            class="fa-solid fa-chevron-right text-[10px] text-textMuted-light group-hover:translate-x-0.5 transition-transform"></i>
                    </a>
                @endif
            </div>
        @else
            <div class="pt-1">
                <a href="{{ url('greetings') }}"
                    class="w-full flex items-center justify-center gap-2 py-2 px-3.5 rounded-xl bg-slate-200/70 dark:bg-slate-800/70 text-xs font-sans font-medium text-textMain-light dark:text-textMain-dark hover:bg-slate-300/70 dark:hover:bg-slate-700/70 transition-all">
                    <i class="fa-solid fa-arrow-left text-xs text-teal-light"></i> Kembali ke Portal
                </a>
            </div>
        @endif
                ...
                if (isset($moduleData['items'])) {
                    foreach ($moduleData['items'] as $chkItem) {
                        ...
                        if (
                            ...
                            ($chkItem['url'] !== '#' && request()->url() === url($pathOnly))
                        ) {
                            $isModuleActive = true;
                            break;
                        }
    ...
    <div class="p-4 bg-transparent shrink-0 mt-auto border-0">
        <div class="flex items-center justify-between gap-2 p-2 rounded-2xl bg-slate-200/50 dark:bg-slate-800/40">
            <div class="flex items-center gap-2.5 min-w-0 flex-1 px-1">
                <div
                    class="w-8 h-8 rounded-xl bg-gradient-to-br from-teal-light to-teal-700 text-white flex items-center justify-center font-sans font-semibold text-xs shrink-0 shadow-md shadow-teal-light/20 border border-teal-500/30">
                    {{ strtoupper(substr(Auth::user()->name ?? 'A', 0, 1)) }}
                </div>
                <div class="min-w-0 flex-1 truncate">
                    <div class="font-sans font-medium text-xs text-textMain-light dark:text-textMain-dark truncate">
                        {{ Auth::user()->name ?? 'Pengguna' }}</div>
                    <div
                        class="font-sans font-medium text-[10px] text-textMuted-light dark:text-textMuted-dark truncate">
                        Administrator</div>
                </div>
            </div>
