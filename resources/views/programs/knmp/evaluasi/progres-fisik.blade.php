...
<div class="mb-6 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
    <div>
        ...
        <p class="text-textMuted-light dark:text-textMuted-dark text-[11px] font-normal mt-1">Evaluasi historis progres konstruksi KNMP — perbandingan rencana vs aktual.</p>
    </div>
    ...
    <form id="evaluasiFilterForm" action="{{ url()->current() }}" method="GET" class="flex flex-wrap items-center gap-3 w-full lg:w-auto">
        ...
        <div class="relative flex items-center bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-700 rounded-xl px-4 py-2 text-xs font-medium focus-within:ring-2 focus-within:ring-teal-light">
            ...
            <input type="date" name="date" value="{{ request('date') ?: $effectiveDate }}" onchange="this.form.submit()" class="bg-transparent border-none outline-none text-textMain-light dark:text-textMain-dark w-32">
        </div>
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
    ...
    <div class="bg-bgSurface-light dark:bg-bgSurface-dark border border-gray-100 dark:border-gray-800 rounded-3xl p-6 relative overflow-hidden group">
        ...
        <div class="flex items-center gap-4 mb-3 relative z-10">
            <div class="w-12 h-12 rounded-xl bg-warning/10 flex items-center justify-center text-warning text-sm"><i class="fa-solid fa-person-digging"></i></div>
            <div>
                <h3 class="text-xs font-medium text-textMuted-light dark:text-textMuted-dark uppercase tracking-wider">Konstruksi Aktif</h3>
                <div class="text-sm font-medium text-warning">{{ $stats['konstruksi_aktif'] }} <span class="text-sm font-medium text-textMuted-light">Lokasi</span></div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-bgSurface-light dark:bg-bgSurface-dark border border-gray-100 dark:border-gray-800 rounded-3xl p-6 relative overflow-hidden group">
        ...
        <div class="flex items-center gap-4 mb-2 relative z-10">
            <div class="w-12 h-12 rounded-xl bg-teal-light/10 flex items-center justify-center text-teal-light text-sm"><i class="fa-solid fa-chart-pie"></i></div>
            <div>
                <h3 class="text-xs font-medium text-textMuted-light dark:text-textMuted-dark uppercase tracking-wider">Rata-Rata Progres</h3>
                <div class="text-sm font-medium text-teal-light">{{ $stats['rata_progres'] }}%</div>
            </div>
        </div>
        <div class="w-full bg-gray-100 dark:bg-gray-800 rounded-full h-2 relative z-10 mt-2">
            <div class="bg-teal-light h-2 rounded-full transition-all" style="width: {{ $stats['rata_progres'] }}%"></div>
        </div>
    </div>
    ...
    <div class="bg-bgSurface-light dark:bg-bgSurface-dark border border-gray-100 dark:border-gray-800 rounded-3xl p-6 relative overflow-hidden group">
        ...
        <div class="flex items-center gap-4 mb-3 relative z-10">
            <div class="w-12 h-12 rounded-xl bg-success/10 flex items-center justify-center text-success text-sm"><i class="fa-solid fa-check-double"></i></div>
            <div>
                <h3 class="text-xs font-medium text-textMuted-light dark:text-textMuted-dark uppercase tracking-wider">Serah Terima</h3>
                <div class="text-sm font-medium text-success">{{ $stats['total_selesai'] }} <span class="text-sm font-medium text-textMuted-light">Lokasi</span></div>
            </div>
        </div>
    </div>
    ...
    {{-- Rata-Rata Deviasi --}}
    <div class="bg-bgSurface-light dark:bg-bgSurface-dark border border-gray-100 dark:border-gray-800 rounded-3xl p-6 relative overflow-hidden group">
        <div class="absolute top-0 right-0 w-32 h-32 {{ $stats['rata_deviasi'] >= 0 ? 'bg-success/10' : 'bg-danger/10' }} rounded-bl-full -mr-8 -mt-8 transition-transform group-hover:scale-110"></div>
        <div class="flex items-center gap-4 mb-3 relative z-10">
            <div class="w-12 h-12 rounded-xl {{ $stats['rata_deviasi'] >= 0 ? 'bg-success/10' : 'bg-danger/10' }} flex items-center justify-center {{ $stats['rata_deviasi'] >= 0 ? 'text-success' : 'text-danger' }} text-sm">
                <i class="fa-solid {{ $stats['rata_deviasi'] >= 0 ? 'fa-arrow-trend-up' : 'fa-arrow-trend-down' }}"></i>
            </div>
            <div>
                <h3 class="text-xs font-medium text-textMuted-light dark:text-textMuted-dark uppercase tracking-wider">Rata-Rata Deviasi</h3>
                <div class="text-sm font-medium {{ $stats['rata_deviasi'] >= 0 ? 'text-success' : 'text-danger' }}">
                    {{ $stats['rata_deviasi'] >= 0 ? '+' : '' }}{{ $stats['rata_deviasi'] }}%
                </div>
            </div>
        </div>
    </div>
...
<div class="bg-bgSurface-light dark:bg-bgSurface-dark border border-gray-100 dark:border-gray-800 rounded-3xl overflow-hidden flex flex-col">
    <div class="p-6 border-b border-gray-100 dark:border-gray-800 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            ...
            <p class="text-xs text-textMuted-light mt-1">Data progres per tanggal <strong>{{ \Carbon\Carbon::parse($effectiveDate)->format('d M Y') }}</strong>.</p>
        </div>
    </div>
    ...
    <div class="px-6 py-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-gray-50/50 dark:bg-gray-800/20">
        ...
        <div class="relative w-full sm:w-64">
            ...
            <input type="text" x-model="searchQuery" @input="currentPage = 1" placeholder="Cari nama lokasi/vendor..." class="w-full pl-8 pr-4 py-2 rounded-md border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-900 text-xs focus:border-teal-light outline-none transition-all">
        </div>
    </div>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800 bg-white dark:bg-bgSurface-dark">
                <template x-for="(item, index) in paginatedData()" :key="index">
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                        <td class="px-6 py-4">
                            <div class="font-medium text-textMain-light dark:text-textMain-dark" x-text="item.nama"></div>
                            <template x-if="item.tahap === 'serah_terima'">
                                <span class="text-[9px] text-success font-medium bg-success/10 px-1.5 py-0.5 rounded mt-0.5 inline-block"><i class="fa-solid fa-check mr-0.5"></i> Selesai</span>
                            </template>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                <div class="w-6 h-6 rounded-full bg-teal-100 dark:bg-teal-900/30 text-teal-light flex items-center justify-center text-[10px] font-bold" x-text="item.konstruktor.substring(0, 2).toUpperCase()"></div>
                                <span class="font-medium text-textMuted-light" x-text="item.konstruktor"></span>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <span class="font-medium text-textMain-light dark:text-textMain-dark" x-text="item.rencana + '%'"></span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex flex-col gap-1.5 w-40">
                                ...
                                <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5">
                                    <div class="h-1.5 rounded-full transition-all" :class="item.progres >= 100 ? 'bg-success' : 'bg-teal-light'" :style="'width: ' + Math.min(item.progres, 100) + '%'"></div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <template x-if="item.deviasi >= 0">
                                <span class="text-success font-medium text-[0.7rem] flex items-center gap-1 bg-success/10 px-2 py-0.5 rounded w-fit">
                                    <i class="fa-solid fa-arrow-up text-[8px]"></i> +<span x-text="item.deviasi"></span>%
                                </span>
                            </template>
                            <template x-if="item.deviasi < 0">
                                <span class="text-danger font-medium text-[0.7rem] flex items-center gap-1 bg-danger/10 px-2 py-0.5 rounded w-fit">
                                    <i class="fa-solid fa-arrow-down text-[8px]"></i> <span x-text="item.deviasi"></span>%
                                </span>
                            </template>
                        </td>
                        <td class="px-6 py-4">
                ...
                <tr x-show="paginatedData().length === 0">
                    <td colspan="7" class="px-6 py-8 text-center text-textMuted-light">Belum ada data progres atau tidak ada hasil pencarian.</td>
                </tr>
            </tbody>
        </table>
    </div>
...
<div x-show="isPdfModalOpen" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
    <div @click.away="isPdfModalOpen = false" x-transition.opacity.duration.200ms class="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-md p-6 shadow-xl border border-gray-100 dark:border-gray-800 relative">
        ...
        <form action="{{ route('program.evaluasi.progres-fisik.pdf', ['program' => 'knmp']) }}" method="GET" target="_blank" @submit="setTimeout(() => isPdfModalOpen = false, 500)">
            <div class="space-y-4">
                <div>
                ...
                <div>
                    ...
                    <input type="date" name="date" value="{{ $effectiveDate }}" class="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-teal-light text-textMain-light dark:text-textMain-dark">
                </div>
            </div>
