...
<div class="mb-6 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
    <div>
        ...
        <p class="text-textMuted-light dark:text-textMuted-dark text-[11px] font-normal mt-1">Evaluasi historis perjalanan proyek KNMP — dari usulan hingga serah terima.</p>
    </div>
...
<div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
    ...
    <div class="bg-bgSurface-light dark:bg-bgSurface-dark border border-gray-100 dark:border-gray-800 rounded-2xl p-4 relative overflow-hidden group hover:shadow-md transition-all">
        <div class="flex items-center gap-3 relative z-10">
            <div class="w-10 h-10 rounded-lg bg-{{ $config['color'] }}/10 flex items-center justify-center text-{{ $config['color'] }} text-xs shrink-0">
                <i class="fa-solid {{ $config['icon'] }}"></i>
            </div>
            <div class="min-w-0">
                <div class="text-[10px] font-medium text-textMuted-light dark:text-textMuted-dark uppercase tracking-wider truncate">{{ $config['label'] }}</div>
                <div class="text-base font-bold text-textMain-light dark:text-textMain-dark">{{ $stats['per_tahap'][$key] ?? 0 }}</div>
            </div>
        </div>
    </div>
    @endforeach
...
<div class="mb-6 bg-gradient-to-r from-teal-light/5 to-blue-500/5 dark:from-teal-light/10 dark:to-blue-500/10 border border-teal-light/20 rounded-2xl p-5 flex flex-col sm:flex-row items-center justify-between gap-4">
    <div class="flex items-center gap-3">
        <div class="w-10 h-10 rounded-full bg-teal-light/20 flex items-center justify-center text-teal-light"><i class="fa-solid fa-chart-simple"></i></div>
        <div>
            <div class="text-xs font-medium text-textMuted-light dark:text-textMuted-dark">Total Lokasi KNMP Terdaftar</div>
            <div class="text-lg font-bold text-textMain-light dark:text-textMain-dark">{{ $stats['total'] }} <span class="text-sm font-normal text-textMuted-light">Lokasi</span></div>
        </div>
    </div>
    <div class="flex items-center gap-4 text-xs">
        <div class="flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-success"></span> <span class="text-textMuted-light">Selesai: <strong class="text-success">{{ $stats['per_tahap']['serah_terima'] ?? 0 }}</strong></span></div>
        <div class="flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-warning"></span> <span class="text-textMuted-light">Dalam Proses: <strong class="text-warning">{{ ($stats['total'] ?? 0) - ($stats['per_tahap']['serah_terima'] ?? 0) }}</strong></span></div>
    </div>
</div>
...
<div class="bg-bgSurface-light dark:bg-bgSurface-dark border border-gray-100 dark:border-gray-800 rounded-3xl overflow-hidden flex flex-col">
    <div class="p-6 border-b border-gray-100 dark:border-gray-800 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            ...
            <p class="text-xs text-textMuted-light mt-1">Tracking perjalanan setiap lokasi KNMP melalui siklus operasional proyek.</p>
        </div>
    </div>
        ...
        <table class="w-full text-left text-xs whitespace-nowrap">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800 bg-white dark:bg-bgSurface-dark">
                ...
                <tr x-show="paginatedData().length === 0">
                    <td colspan="5" class="px-6 py-8 text-center text-textMuted-light">Belum ada data atau tidak ada hasil pencarian.</td>
                </tr>
            </tbody>
        </table>
    </div>
...
<div x-show="isPdfModalOpen" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
    <div @click.away="isPdfModalOpen = false" x-transition.opacity.duration.200ms class="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-md p-6 shadow-xl border border-gray-100 dark:border-gray-800 relative">
        ...
        <form action="{{ route('program.evaluasi.operasional.pdf', ['program' => 'knmp']) }}" method="GET" target="_blank" @submit="setTimeout(() => isPdfModalOpen = false, 500)">
            <div class="space-y-4">
                <div>
