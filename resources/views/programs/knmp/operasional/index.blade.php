...
<div class="mb-6 flex flex-col gap-4">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            <button class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 hover:bg-gray-50 text-textMain-light dark:text-textMain-dark rounded-md px-4 py-2 text-xs font-medium shadow-sm transition-all flex items-center justify-between gap-2"> Import Progres Massal <i class="fa-solid fa-cloud-arrow-up text-teal-light"></i> </button>
            @endif
        </div>
    </div>

    <!-- Stepper / Tabs -->
    <div class="bg-white dark:bg-bgSurface-dark rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 p-2 overflow-x-auto hide-scrollbar">
        <div class="flex min-w-max">
            @foreach($stages as $key => $data)
            <a href="?stage={{ $key }}" class="flex items-center group relative px-4 py-3">
                <div class="flex items-center gap-3 relative z-10">
                    <div class="w-8 h-8 rounded-full flex items-center justify-center font-medium text-xs transition-colors
                    ...
                    <span class="font-medium text-sm transition-colors {{ $currentStage === $key ? 'text-success' : 'text-textMuted-light dark:text-textMuted-dark group-hover:text-success' }}">
                        {{ $data['label'] }}
                    </span>
                </div>
                @if(!$loop->last)
...
<div class="bg-bgSurface-light dark:bg-bgSurface-dark border border-gray-100 dark:border-gray-800 rounded-3xl shadow-sm overflow-hidden flex flex-col">
    ...
    <div class="p-4 border-b border-gray-100 dark:border-gray-800 flex flex-col sm:flex-row justify-between items-center gap-4 bg-gray-50/50 dark:bg-gray-800/20">
        <div class="flex flex-wrap items-center gap-4 w-full sm:w-auto">
            <div class="flex items-center gap-2 text-sm text-textMuted-light dark:text-textMuted-dark">
                <span>Tampilkan</span>
                <select class="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-700 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:border-teal-light text-textMain-light dark:text-textMain-dark font-medium shadow-sm">
                    <option>10</option>
                    <option>25</option>
                </select>
                <span>entri</span>
            ...
            <button class="bg-success hover:bg-emerald-600 text-white rounded-md px-4 py-1.5 text-xs font-medium shadow-sm hover:shadow transition-all flex items-center gap-2">
        ...
        <div class="relative w-full sm:w-64">
            ...
            <input type="text" placeholder="Cari proyek..." class="w-full pl-9 pr-4 py-2 rounded-xl border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm focus:border-success outline-none transition-all shadow-sm">
        </div>
    </div>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800 bg-white dark:bg-bgSurface-dark">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors group cursor-pointer">
                    ...
                    @elseif($currentStage === 'konstruksi' || $currentStage === 'serah-terima')
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-xs font-medium">{{ $prog }}%</span>
                            </div>
                        </td>
                    @endif
                    ...
                    <td class="px-6 py-4">
                        ...
                        @else
                        <span class="px-2.5 py-1 rounded-md text-[0.7rem] font-medium bg-teal-light/10 text-teal-light">
                            Sedang Berjalan
                        </span>
                        @endif
                    </td>
                    <td class="px-6 py-4 text-center">
                        <div class="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button class="w-8 h-8 rounded-md bg-teal-light/10 text-teal-light hover:bg-teal-light hover:text-white transition-colors flex items-center justify-center" title="Update Progres">
                                <i class="fa-solid fa-pen-to-square"></i>
                            </button>
                            <button class="w-8 h-8 rounded-md bg-success/10 text-success hover:bg-success hover:text-white transition-colors flex items-center justify-center" title="Lampirkan Dokumen">
    ...
    <div class="px-6 py-4 border-t border-gray-100 dark:border-gray-800 flex justify-between items-center text-sm bg-gray-50/50 dark:bg-gray-800/20">
        ...
        <div class="flex gap-1 shadow-sm rounded-lg overflow-hidden border border-gray-100 dark:border-gray-700">
            <button class="w-9 h-9 bg-success text-white font-medium flex items-center justify-center">1</button>
        </div>
    </div>
</div>
@endsection
