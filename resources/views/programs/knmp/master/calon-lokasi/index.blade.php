...
@section('content')
 ...
 {{ $currentStage === $key ? 'bg-teal-light text-white ' : 'bg-gray-100 dark:bg-gray-800 text-gray-400 group-hover:bg-teal-light/20 group-hover:text-teal-light' }}">
        ...
        <div>
            ...
            <div x-show="showDetailDataModal" style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm"
                 x-transition:enter="transition ease-out duration-300"
                 x-transition:enter-start="opacity-0"
                 x-transition:enter-end="opacity-100"
                 x-transition:leave="transition ease-in duration-200"
                ...
                <div @click.away="showDetailDataModal = false"
                    ...
                    <div class="flex justify-between items-center mb-6 shrink-0">
                        <div>
                            <h3 class="text-lg font-semibold text-textMain-light dark:text-textMain-dark flex items-center gap-2">
                                <i class="fa-solid fa-map-location-dot text-teal-light"></i> Rincian Calon Lokasi
                            </h3>
                            <p class="text-xs text-textMuted-light mt-0.5" x-text="activeDetail ? `${activeDetail.desa}, ${activeDetail.kecamatan}, ${activeDetail.kabupaten}, ${activeDetail.provinsi}` : ''"></p>
                        </div>
                        <button type="button" @click="showDetailDataModal = false" class="text-gray-400 hover:text-danger transition-colors">
                            <i class="fa-solid fa-xmark text-lg"></i>
                        </button>
                    </div>
                            ...
                            <div class="space-y-6">
                                ...
                                <div>
                                    ...
                                    <div class="mt-6">
                                        ...
                                        <div>
                                            ...
                                            <div class="flex flex-row flex-wrap gap-1.5">
                                                <span class="px-2 py-0.5 rounded text-[10px] font-medium transition-colors"
                                                    :class="activeDetail.detail.is_mangrove === 'Ya' ?
                                                        'bg-success/10 text-success' :
                                                        'bg-gray-50 text-gray-400 border border-gray-100'">Area
                                                    Mangrove</span>
                                                <span class="px-2 py-0.5 rounded text-[10px] font-medium transition-colors"
                                                    :class="activeDetail.detail.is_konservasi === 'Ya' ?
                                                        'bg-success/10 text-success' :
                                                        'bg-gray-50 text-gray-400 border border-gray-100'">Zona
                                                    Konservasi</span>
                                                <span class="px-2 py-0.5 rounded text-[10px] font-medium transition-colors"
                                                    :class="activeDetail.detail.is_hutan_lindung === 'Ya' ?
                                                        'bg-success/10 text-success' :
                                                        'bg-gray-50 text-gray-400 border border-gray-100'">Hutan
                                                    Lindung</span>
                                                <span class="px-2 py-0.5 rounded text-[10px] font-medium transition-colors"
                                                    :class="activeDetail.detail.is_kawasan_budidaya === 'Ya' ?
                                                        'bg-success/10 text-success' :
                                                        'bg-gray-50 text-gray-400 border border-gray-100'">Kawasan
                                                    Budidaya</span>
                                                <span class="px-2 py-0.5 rounded text-[10px] font-medium transition-colors"
                                                    :class="activeDetail.detail.is_das === 'Ya' ?
                                                        'bg-success/10 text-success' :
                                                        'bg-gray-50 text-gray-400 border border-gray-100'">Area
                                                    DAS</span>
                                            </div>
                                    ...
                                    <div class="mt-6 pt-4 border-t border-gray-100 dark:border-gray-800">
                                        <div class="text-[10px] text-gray-400 uppercase tracking-wider font-medium mb-2">
                                            Tautan Dokumen Pendukung</div>
                                        <template x-if="activeDetail.dokumen">
                                            <a :href="activeDetail.dokumen" target="_blank"
                                                class="inline-flex items-center gap-2 px-4 py-2 bg-teal-light/10 text-teal-light rounded-lg text-xs font-medium hover:bg-teal-light hover:text-white transition-colors">
