...
@section('content')
    <div x-data="{
        ...
        @if (session('success'))
            <div
                class="mb-4 p-4 rounded-xl bg-success/10 border border-success/20 text-success text-sm flex items-center gap-3">
        ...
        @if (session('error'))
            <div
                class="mb-4 p-4 rounded-xl bg-danger/10 border border-danger/20 text-danger text-sm flex items-center gap-3">
        ...
        <div x-show="showModal" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div x-show="showModal" x-transition:enter="ease-out duration-300"
                    ...
                    class="relative z-10 inline-block align-bottom bg-bgSurface-light dark:bg-bgSurface-dark rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md w-full border border-gray-100 dark:border-gray-800 p-6">
                    <form
                        :action="isEdit ? '{{ url('master/knmp/batch') }}/' + formData.id :
                            '{{ route('program.master.batch.store') }}'"
                        method="POST">
                        @csrf
                        ...
                        <div class="space-y-4">
                            <div>
                                <label
                                    ...
                                    Tahap <span class="text-danger">*</span></label>
                                <div class="relative">
                                    <i
                                        class="fa-solid fa-layer-group absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-xs"></i>
                                    <input type="text" name="nama_tahap" x-model="formData.nama_tahap" required
                                        placeholder="Contoh: I, II, III, Susulan"
                                ...
                                <p class="text-[10px] text-textMuted-light mt-1.5">Masukkan nama atau penomoran tahap
                                    (misal: "I" atau "Susulan").</p>
                            </div>
                            <div>
                                <label
                                    class="block text-xs font-medium text-textMuted-light dark:text-textMuted-dark mb-1.5">Tahun
                                    <span class="text-danger">*</span></label>
                                <div class="relative">
                                    <i
                                        class="fa-regular fa-calendar absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-xs"></i>
                                    <input type="number" name="tahun" x-model="formData.tahun" required
                                        min="2000" max="2099" placeholder="Contoh: 2025"
                                        class="w-full pl-11 pr-4 py-2.5 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-sm outline-none focus:border-teal-light focus:ring-1 focus:ring-teal-light transition-all text-textMain-light dark:text-textMain-dark">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-8 flex justify-end gap-3">
                            ...
                            <button type="submit"
                                class="px-4 py-2 bg-teal-light text-white rounded-lg text-sm font-medium hover:bg-teal-600 transition-colors flex items-center gap-2">
